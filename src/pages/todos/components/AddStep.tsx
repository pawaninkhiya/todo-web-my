...
import { useAddStepToTodoMutation, useDeleteStepFromTodoMutation, useUpdateStepToTodoMutation } from '@services/apis/todos/hooks';
...
const AddStep: React.FC<AddStepProps> = memo(({ initialEditData, refetch }) => {
    ...
    const handleUpdateStep = useCallback(async (stepData: Partial<Step>) => {
    ...
    const toggleStepCompletion = useCallback(async (step: Step) => {
    ...
    const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
        if (e.key === 'Enter') {
            editingStep ? handleUpdateStep({ title: stepTitle.trim() }) : handleAddStep();
        } else if (e.key === 'Escape') {
            resetForm();
        }
    }, [editingStep, stepTitle, handleUpdateStep, handleAddStep, resetForm]);

    const startEditing = useCallback((step: Step) => {
            ...
            {initialEditData?.steps?.map((step) => (
                ...
                >
                    <div className="flex items-center gap-3 w-full">
                        ...
                        >
                            <Icons.Cross fontSize={15} className="text-red-500" />
                        </button>
            ...
            {isAddingStep && (
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            type="button"
                            className={`text-xs text-white cursor-pointer transition-colors p-1 rounded-full ${stepTitle.trim() ? 'bg-blue-500 hover:bg-blue-600' : 'bg-gray-300 cursor-not-allowed'}`}
                            onClick={handleSubmit}
                            disabled={!stepTitle.trim()}
                            aria-label={editingStep ? 'Update step' : 'Add step'}
                        >
                            <Icons.Check size={14} />
                        </button>
                    </div>
