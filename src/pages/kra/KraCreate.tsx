...
import {useCreateKRAMutation,useGetKRAByIdQuery,useUpdateKRAMutation} from "@services/apis/KPA/hooks";
...
const DEFAULT_FORM_DATA: KRAFormData = {
    title: "",
    description: "",
    assignTo: "",
};
...
const KraCreate = () => {
    ...
    const [formData, setFormData] = useState<KRAFormData>(DEFAULT_FORM_DATA);
    ...
    }, [existingKpa, dropdownUsers, kraId]);
    ...
    const handleClear = () => {
        setFormData(DEFAULT_FORM_DATA);
        // Don't clear selectedUser if we're editing an existing KRA
        if (!kraId) {
            setSelectedUser(null);
        }
        setKraId(null);
    ...
    const config = {
        ...editorConfig,
        placeholder: formData.description?.trim() ? "" : "Enter your KPA description here...",
    };
    return (
