...
import { useGetAllKRAQuery } from "@services/apis/KPA/hooks";
...
const SidePanel = ({ onKraSelect, handleSidePanelToggle, isSidePanelOpen, assignedUserId, search, setSearch }: SidePanelProps) => {
    const debouncedSearch = useDebounceValue(search, 500);
    const { data, isPending, isError } = useGetAllKRAQuery({
        assignedUserId: assignedUserId ?? "",
        search: assignedUserId ? "" : debouncedSearch ?? "",
    });

    useEffect(() => {
        if (assignedUserId && data?.data?.length) {
            const existingKRA = data.data.find(
                (kra: any) => kra.assignTo?._id === assignedUserId
            );
            if (existingKRA) {
                setSearch?.("");
                onKraSelect(existingKRA._id);
            }
        }
    }, [assignedUserId, data]);
    ...
    return (
        <div
            className={`w-full max-w-full xl:max-w-[370px] bg-white shadow-lg border-l border-gray-200 flex flex-col fixed top-0 right-0 h-full xl:static xl:h-auto z-50 xl:shadow-none transition-transform duration-300 ease-in-out scrollbar-hide ${isSidePanelOpen ? "translate-x-0" : "translate-x-full"
                } xl:translate-x-0`}
        >
            {/* Header */}
            <div className="sticky top-0 bg-white z-10 flex items-center justify-between p-4 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-800">KRA List</h2>
                <button className="xl:hidden text-gray-500 hover:text-gray-700" onClick={handleSidePanelToggle}>
                    <Icons.Cross />
                </button>
            </div>
                ...
                <div className="p-4">
                    {isPending ? (
                        ...
                        <div className="p-4 bg-red-50 rounded-lg text-center">
                            <p className="text-red-600 text-sm">Failed to load data. Please try again later.</p>
                        </div>
                    ) : data?.data?.length ? (
                        <div className="space-y-3">
                            {data.data.map((kpa: any) => (
                                <div
                                    key={kpa._id}
                                    className="p-4 rounded border cursor-pointer transition-colors group relative hover:border-purple-300 hover:bg-purple-50 bg-gray-50 border-gray-200"
                                    onClick={() => onKraSelect(kpa._id)}
                                >
                                    <h3 className="font-medium text-gray-800 text-sm mb-2 line-clamp-2 max-w-[90%]">{kpa.title}</h3>
                        ...
                        <div className="p-6 text-center">
                            <p className="text-gray-400 text-sm">No KRAs available. Create one to get started.</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="p-4 border-t border-gray-200 bg-white">
                <p className="text-xs text-gray-500 text-center">Showing {data?.data?.length || 0} items</p>
            </div>
        </div>
    );
};
